...
let cfg = {
  asar: true,
  extraResource: ['src/bin', 'src/images', 'src/app-update.yml', 'src/codex-seed'],
...
if (process.env.APPLE_TEAM_ID) {
  cfg.osxSign = {
    ...
    entitlements: 'config/electron/entitlements.plist',
    'entitlements-inherit': 'config/electron/entitlements.plist',
  };
  cfg.osxNotarize = {
  ...
  makers: [
    ...
    },
    {
      ...
      config: {
        ...
        desktopTemplate: './config/electron/forge.deb.desktop',
        options: {
    ...
    },
    {
      ...
      config: {
        ...
        homepage: 'https://goose-docs.ai/',
        categories: ['Development'],
        desktopTemplate: './config/electron/forge.rpm.desktop',
        options: {
          icon: 'src/images/icon.png',
          prefix: '/opt',
  ...
  plugins: [
    {
      ...
      config: {
        build: [
          {
            entry: 'src/main.ts',
            config: 'config/vite/vite.main.config.mts',
          },
          {
            entry: 'src/preload.ts',
            config: 'config/vite/vite.preload.config.mts',
          },
        ],
        renderer: [
          {
            name: 'main_window',
            config: 'config/vite/vite.renderer.config.mts',
          },
        ],
      },
    },
