...
The Claude Code (`claude-code`) and Gemini CLI (`gemini-cli`) providers are deprecated. Use the [ACP providers](/docs/guides/acp-providers) instead, which support goose extensions via MCP and use the standardized Agent Client Protocol. For Gemini, use the `Gemini` (`gemini_oauth`) provider which authenticates via OAuth. CLI providers are kept for backward compatibility only.
:::
...
- **Claude Code**: Converts goose messages to text content blocks with role prefixes (Human:/Assistant:), similar to Gemini CLI
