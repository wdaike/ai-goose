...
describe('mapThreadToMessages', () => {
  ...
  it('groups phase-less progress messages while leaving the trailing answer messages outside', () => {
    const items = [
      {
        type: 'userMessage',
        id: 'user-1',
      ...
      { type: 'reasoning', id: 'reasoning-1', summary: ['Summarizing'], content: [] },
      {
        type: 'agentMessage',
        id: 'answer-intro-1',
        text: 'Below is the architecture summary.',
        phase: null,
        memoryCitation: null,
      },
      {
    ...
    expect(byId.get('command-1')?.metadata.workGroupId).toBe('work-user-1');
    expect(byId.get('answer-intro-1')?.metadata.workGroupId).toBeUndefined();
    expect(byId.get('answer-1')?.metadata.workGroupId).toBeUndefined();
  });

  it('treats a trailing answer followed by reasoning as the answer', () => {
    const items = [
      {
        type: 'userMessage',
        id: 'user-1',
      ...
      { type: 'reasoning', id: 'reasoning-1', summary: ['Wrapping up'], content: [] },
    ] as ThreadItem[];
  ...
  it('folds all but the last text of restored threads that lost their tool items', () => {
    ...
    const items = [
      ...
      { type: 'reasoning', id: 'reasoning-1', summary: ['Exploring'], content: [] },
      {
        type: 'agentMessage',
        id: 'progress-1',
        text: 'Now let me explore the core modules.',
        phase: null,
        memoryCitation: null,
      },
      { type: 'reasoning', id: 'reasoning-2', summary: ['Summarizing'], content: [] },
      {
        type: 'agentMessage',
        id: 'answer-1',
