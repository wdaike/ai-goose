...
import { MessageQueue, QueuedMessage } from '../conversation/MessageQueue';
import { useMessageQueue } from '../../hooks/useMessageQueue';
import { IDLE_CURSOR, navigateHistory, type HistoryCursor } from '../../codex/engine/commandHistory';
...
export default function ChatInput({
  ...
  const {
    ...
    remove: handleRemoveQueuedMessage,
    clear: handleClearQueue,
    reorder: handleReorderMessages,
  ...
  const [historyCursor, setHistoryCursor] = useState<HistoryCursor>(IDLE_CURSOR);
  ...
  const handleHistoryNavigation = (evt: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    const savedDraft = historyCursor.index === -1 ? displayValue || '' : savedInput;
    const navigation = navigateHistory({
      direction: isUp ? 'up' : 'down',
      cursor: historyCursor,
      savedDraft,
      sessionHistory: commandHistory,
      globalHistory: LocalMessageStorage.getRecentMessages() || [],
    });
    if (!navigation) return;

    if (historyCursor.index === -1) {
      setSavedInput(savedDraft);
    }
    setHistoryCursor(navigation.cursor);
    setDisplayValue(navigation.value);
  ...
  const handleInterruptionAndQueue = () => {
    ...
    const interruptionMatch = detectInterruption(contentToQueue);
    if (interruptionMatch && interruptionMatch.shouldInterrupt) {
      // Queued rather than sent: the stop has to land before the turn can restart
      enqueueInterruption(message, interruptionMatch.matchedText);
    } else {
  ...
  const performSubmit = useCallback(
    (text?: string) => {
      ...
      if (textToSend || imageData.length > 0 || attachedFiles.length > 0) {
        ...
        if (textToSend && !detectInterruption(textToSend)) {
          resumeAfterNonInterruption();
        }
