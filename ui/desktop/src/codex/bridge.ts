...
const CODEX_HOME = path.join(os.homedir(), '.goose');
const LEGACY_GOOSE_CONFIG = path.join(os.homedir(), '.config', 'goose', 'config.yaml');
const GLOBAL_CODEX_HOME = path.join(os.homedir(), '.codex');
...
function ensureCodexHome(): string {
  ...
  if (!fs.existsSync(configPath)) {
    ...
    log.info(`Created ${configPath} from legacy goose config`);
  }

  const authPath = path.join(CODEX_HOME, 'auth.json');
  const globalAuthPath = path.join(GLOBAL_CODEX_HOME, 'auth.json');
  if (!fs.existsSync(authPath) && fs.existsSync(globalAuthPath)) {
    fs.copyFileSync(globalAuthPath, authPath);
    log.info('Copied codex auth.json into goose CODEX_HOME');
  }
...
/**
 * Spawns `codex app-server` and bridges its stdio JSONL JSON-RPC to the
 * renderer over IPC. The renderer speaks the app-server v2 protocol directly;
 * this bridge only does transport, request correlation, and fan-out.
 */
export class CodexBridge {
  ...
  start(): void {
    ...
    child.on('exit', (code, signal) => {
      log.error(`codex app-server exited code=${code} signal=${signal}`);
      const error = new Error(`codex app-server exited (code=${code})`);
      for (const pending of this.pending.values()) pending.reject(error);
      this.pending.clear();
      this.child = null;
      this.initialized = null;
      this.broadcast({ method: 'goose/codexExited', params: { code, signal } });
    });

    child.stderr?.on('data', (data: Buffer) => {
      log.info(`[codex] ${data.toString().trimEnd()}`);
    });
    ...
    this.initialized = this.request('initialize', {
      clientInfo: { name: 'goose', title: 'goose', version: process.env.npm_package_version ?? '0.0.0' },
  ...
  private onLine(line: string): void {
    ...
    try {
      ...
      log.error(`codex app-server: unparseable line: ${line.slice(0, 200)}`);
      return;
    }
    ...
    // both go straight to the renderer; the renderer answers requests via
    // `codex:respond`.
    this.broadcast(msg);
  }
